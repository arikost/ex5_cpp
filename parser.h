    ...
    void add_counter(string type, string name){
        ...
        for (int i = 0; i < _counters.size(); ++i) {
            if(_counters[i]->getType() == typeid(int).name()){
                Counter<int> *counter = static_cast<Counter<int>* >(_counters[i]);
                currentName = counter->getName();
            }
            else{
                Counter<string> *counter = static_cast<Counter<string>* >(_counters[i]);
                currentName = counter->getName();
            }
        ...
        if(type == "int_counter"){
            Counter<int> *ci = new Counter<int>(name);
            _counters.push_back(ci);
        }
